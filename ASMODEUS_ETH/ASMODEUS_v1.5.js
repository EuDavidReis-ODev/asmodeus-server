...
const dest_addr = 'ENDEREÇO DA WALLET DESTINO, PARA ONDE SERA ENVIADO O ETH';

var web3 =  new Web3(new Web3.providers.WebsocketProvider(providerTest));
...
function setTransactionDetails(resEstimateGas, gasP) {
            ...
            console.log("Valor em conta::"+wte(balance));
            amount = balance - (gasP*resEstimateGas);
            console.log("GasFee::"+(gasP*resEstimateGas));
            console.log(wte(balance)+"-("+gasP+"(gwei)*"+resEstimateGas+"(gwei))="+wte(amount));
            console.log("Valor a ser enviado::"+wte(amount));
...
function wte(valueInEther){
    valueInEther =""+valueInEther;
    valueInEther = web3.utils.fromWei(valueInEther, 'ether')+"ETH";
    return valueInEther;
}
